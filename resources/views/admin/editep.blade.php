...
<br>
    ...
    <div class="container">
        <a href="{{ url('') }}/admin/backupfind/{{ $vide_find_ep['v_id'] }}" class="btn btn-danger"> <i class="fas fa-caret-square-left"></i> Back</a>
        <br><br>
        <div class="row">
            <div class="col-md-2">
        ...
        @if(session('success'))
            ...
            <br>
        @endif
        <form action="{{ route('leteditep') }}" method="POST">
            @csrf
            <div class="box" style="color:white;">
                Title:
                <input type="text" name="a_name" class="form-control" style="background:#364064;border-color:#364064;color:white;width:60%;" value="{{ $ep['a_name'] }}" required>
                Iframe:
                <textarea  name="a_iframe" id="" cols="30" rows="5" class="form-control" style="background:#364064;border-color:#364064;color:white;">{{ $ep['a_iframe'] }}</textarea>
                <input type="hidden" name="a_id" value="{{ $ep['a_id'] }}">
                <br>
    ...
    <?php
            if($series == "ok"){
                ...
                $cut_name = str_replace(' ','+',$ep['a_name']);
